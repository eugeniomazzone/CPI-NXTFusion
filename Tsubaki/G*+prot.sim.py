...
def main(args):
	...
	print("Done")
	############ preparing sparse prot prot similarity
	...
	for line in data:
		...
		if found==True and len(line.strip().split())!=0:
			if line.strip().split()[0][:10] == '>SEQUENCE_': 
				target = line.strip().split()[0][10:]
			if line.strip().split()[0:]==['Score','=']:
				score = int(line.strip().split()[3][1:-3])/100
				dic[int(query),int(target)]=score
		if len(line.strip().split())!=0:
			if line.strip().split()[0]=='Query=':
				query=line.strip().split()[1][9:]
				found=True
