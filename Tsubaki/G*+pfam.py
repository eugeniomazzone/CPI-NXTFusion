...
def main(args):
	...
	for i in dic.keys():
		...
		pf.append(i[1])
		values.append(dic[i])
	pfam_train=sp((values,(pt,pf)), shape=shape2)
	############# train ########################
	...
	protPfamLoss = L.LossWrapper(t.nn.MSELoss(), type="regression", ignore_index = IGNORE_INDEX)
...
class Model(NXmodelProto):
	def __init__(self, ERG, name):
		...
		self.pfamHid = t.nn.Sequential(t.nn.Linear(PFAM_LATENT_SIZE, 20), t.nn.LayerNorm(20), ACTIVATION())
		self.biProtPfam = t.nn.Bilinear(20, 20, 10)
		self.outProtPfam = t.nn.Sequential( t.nn.LayerNorm(10), ACTIVATION(), t.nn.Dropout(0.3), t.nn.Linear(10,1),t.nn.Sigmoid())
